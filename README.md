...
Back to our Mythic VM in the cloud we can SSH into it with you CLI of choise with the credentials listed on your cloud provider.  And, just like before, we'll updated it with the following `apt update && apt upgrade -y`.

Now we can start installing our prerequisites for Mythic.
- **Docker Compose** - `apt install docker-comp0ose`
